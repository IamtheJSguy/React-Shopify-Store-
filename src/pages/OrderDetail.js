...
React.useEffect(() => {
 ...
 getOrders();
  ...
  return (
    ...
    <Typography style={{marginTop:20,textAlign:'center'}} variant="h5">
        Order Details
    </Typography>
      <Divider/>
      <Box
  ...
  <Table sx={{ minWidth: 650 }} aria-label="simple table" >
    <TableHead style={{backgroundColor:'#0288d1'}}>
      ...
      <TableCell style={{color:'white',fontWeight:'bold'}} align="left">UserID</TableCell>
        ...
        <TableCell style={{color:'white',fontWeight:'bold'}}>Bill (PKR)</TableCell>
        ...
        >
          ...
          <TableCell component="th" scope="row">
            <select  onChange={(e)=>handleStatus(e,row._id)} >
              <option disabled={row?.status == 'Delivered'} value={row?.status}>{row.status === "Unpaid" ? "Dispatched": row.status || 'Dispatched'} </option>
              <option disabled={row?.status == 'Delivered'} value="Delivered">Delivered</option>
            </select>
          </TableCell>
        </TableRow>
      ))}
